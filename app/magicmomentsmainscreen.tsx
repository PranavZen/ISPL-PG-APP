...
const MagicMomentsMainScreen = () => {
  const [magicMomentsVideos, setMagicMomentsVideos] = useState<MagicMomentsVideo[]>([]);
    ...
    fetch("https://my.ispl.popopower.com/api/video-master/all-vedios")
      ...
      .then((data) => {
        const filteredVideos = data.data["all-video"].filter(
          (video: MagicMomentsVideo) => video.category_names === "Magic-Moments"
        );
        setMagicMomentsVideos(filteredVideos);
      })
    ...
    <ScrollView showsHorizontalScrollIndicator={false} style={styles.container}>
      <View style={styles.grid}>
        {loading ? (
          ...
          magicMomentsVideos.map((video, index) => {
            ...
            return (
              <HighlightsCard
                ...
                backgroundImg={`https://my.ispl.popopower.com/images/videos/thumbnail/${video.thumbnail}`}
                date={video.date}
...
export default MagicMomentsMainScreen;
